...
<p class="txt-center"><img src="/images/posts/celtic-and-slavic-initial-letters.jpg" alt="Celtic and Slavonic initials" loading="lazy"><br>
<small>Celtic (on the left) and ancient Slavic initials</small></p>
<p>Another example: small pictures of the site's mascot could lead the visitor to the conversion page. Finally, with the help of themed icons, it's easy to emphasize the key paragraphs of the article.</p> 
...
<blockquote>Probably everybody knows that <b>manga means</b> the cartoon story: like comics, but not necessarily funny. It may be sad, heartwarming, cruel, and whatsoever. Manga was born in Japan, but now people draw these cartoons throughout the world. Based on popular episodes, they write novels and produce full-length series of anime.</blockquote>
<p>For our purpose, we'll use only<span class="under"> free online tools</span> (details will appear as we go along):</p>
<ul>
...
<p class="txt-center"><img loading="lazy" src="/images/posts/before_1.png" alt="The source PNG no.1"> &thinsp; <img loading="lazy" src="/images/posts/before_2.png" alt="The source PNG no.2"></p>
...
<p>Already have a <span itemprop="supply" itemtype="http://schema.org/HowToSupply" itemscope><span itemprop="name">proper PNG</span></span>? Let's start with the following steps. You can go immediately to <a href="#p_5">step 5</a>, but for the best outcome, it is advisable to work a little with the drawing.</p>
<ol class="emphasis">
  ...
  <li itemprop="step" itemscope itemtype="http://schema.org/HowToStep"><span itemprop="text">Look at the dark-colored areas: if there are lighter pieces inside, the converter may conclude that they are &ldquo;white&rdquo;. It's better to make them darker by hand, and the accuracy is not very important. The main thing is not to go beyond the contours. Save the resulting PNG; it will look like these:</span>
    <ul class="nobullets oneline txt-center"><li><img loading="lazy" src="/images/posts/after-the-3rd-step_1.jpg" alt="Png no.1 after step 3"> &thinsp; </li><li><img loading="lazy" src="/images/posts/after-the-3rd-step_2.jpg" alt="Png no.2 after step 3"></li></ul>
...
<p>But what would happen if we converted the cat into SVG <a href="/assets/post-on-icons/improper_1.svg" rel="nofollow">without preparation</a> and did not optimize the result: an extra 20&#8239;KB + unsuitability for use. The original manga boy PNG is more contrasting, so the raw SVG from it <a href="/assets/post-on-icons/improper_2.svg" rel="nofollow">would have turned out</a> to be sharp but also overloaded with trifles.</p>
</div>
...
<blockquote>Webfont isn't the only way to use icons: single SVG can be loaded from an external file or embedded into HTML. It's also a common practice to use <a href="#3">SVG sprites</a>. There are many ways to use custom icons, but each one has limitations.</blockquote>
...
<ul class="featured plus">
  ...
  <li>We can apply to the icons the same CSS effects as to surrounding text (color, text-shadow, etc.).</li>
</ul>
...
<ol start="10" class="emphasis">
  <li><span>Select the application and go to the site. Look closely at the available icons: perhaps some of them will come in handy. Select the necessary ones.</span></li>
  ...
  <ul class="nobullets txt-center"><li><img loading="lazy" src="/images/posts/icomoon-vs-fontello.jpg" alt="Contents of archives generated by fontello and icomoon"></li><li><small>Font weight difference: IcoMoon's .eot and .ttf are lighter then Fontello's; .svg font, on the contrary, is heavier.</small></li></ul>
  </li>
...
<div>
<p>It's time to show all the manga icons. They are collected in the <strong>sprite</strong> &ndash; general SVG file containing elements</p> 
...
<p>Here I should present the same icons as the webfont characters, but I don't want to overload the page. As a demo, we'll use the main iconic font which must be downloaded anyway. It's easy to treat our manga font similarly.</p>
...
<p>If your new webfont is just an alphabet used for drop caps, then you need to replace <code>.icon</code> selector with <code>p:first-of-type::first-letter</code> in the stylesheets. The content of pseudo-element <code>::before</code>&thinsp; is no longer needed.</p>
  ...
  <div class="editor">
    <div id="palette" class="flex-wrap flex-center">
      <div>
        ...
        <div id="hide-palette" class="red"><b title="Close the palette" class="option">Close</b><b class="option hidden">Palette</b></div>
      </div>
...
<div class="flex-wrap flex-center">
<div class="css"><label for="svg-info">CSS styles for an SVG icon</label>
<textarea wrap="hard" cols="36" spellcheck="false" id="svg-info">.icon-cat3 {
  fill: #272822;
  width: 200px; height: 200px;
}</textarea>
</div>
<div class="css"><label for="icon-info">CSS for a webfont character</label>
