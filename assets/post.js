...
var app = "https://dyos.fly.dev",
    repo = "do-your-own-seo/do-your-own-seo.github.io",
    branch = "/source/",
...
/* Adding GA event to capture clicks on outbound links */
    document.getElementsByClassName('card')[0].addEventListener('click', function(e){
      var outbound = e.target;
      if (outbound.tagName == 'A' &&  outbound.hostname !== location.hostname && ga && ga.create) {
        var url = outbound.href, 
          options = { 'dimension2': url };
        if ( outbound.target !== '_blank' && !navigator.sendBeacon) {
          e.preventDefault();
          options.hitCallback = function(){ location.href = url }
        }
